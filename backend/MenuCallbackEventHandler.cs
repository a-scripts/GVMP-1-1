...
namespace VMP_CNR
{
    public class MenuCallbackEventHandler : Script
    {
        ...
        [RemoteEvent]
        public void m(Player player, int index)
        {
            try
            {
                ...
                else if (menuid == Dialogs.menu_shop_clothes)
                {
                    ...
                    else if (index == 1)
                    {
                        ...
                        if (price > 0)
                        {
                            ...
                            {
                                if (!iPlayer.TakeCWS((CWSTypes)clothesShop.CWSId, price))
                                {
                                    iPlayer.SendNewNotification(MSG.Money.NotEnoughCW(price, (CWSTypes)clothesShop.CWSId));
                                    return;
                                }
                                else
                                {
                        ...
                        ClothesShopModule.Instance.Buy(iPlayer, null);
                ...
                else if (menuid == Dialogs.menu_shop_mechanic)
                {
                    ...
                    if (index == 3)
                    {
                        if (!iPlayer.Container.CanInventoryItemAdded(591, 1))
                        {
                            iPlayer.SendNewNotification("Nicht genug Platz im Inventar!");
                            return;
                        }
                        ...
                        iPlayer.Container.AddItem(591, 1);
                        return;
                    }
                    if (index == 4)
                    {
                    ...
                    switch (index)
                    {
                        ...
                        case 2:
                            if (iHouse.MoneyKeller > 0)
                            {
                                ...
                                {
                                    iPlayer.SendNewNotification("Die Geldwäsche ist aufgrund von Problemen deaktiviert. Wir arbeiten an der Lösung des Problems. Sobald dieses Feature wieder verfügbar ist," +
                                        "geben wir im Forum und/oder GVRP-Launcher Bescheid.");
                                    DialogMigrator.CloseUserMenu(player, Dialogs.menu_house_keller);
                                    return;
                                }
                                ...
                                foreach (SxVehicle Vehicle in VehicleHandler.Instance.GetAllVehicles())
                                {
                                        ...
                                        garage.Spawns.First().Position.DistanceTo(Vehicle.entity.Position) <= 15.0f)
                                    {
                                            if (idx == index - 2)
                                            {
                                                ...
                                                iPlayer.SendNewNotification("Fahrzeug wurde in die Garage geparkt!");
                                                return;
                                            }
                                        idx++;
                                    }
                ...
                else if (menuid == Dialogs.menu_shop_ammunation_main)
                {
                    switch (index)
                    {
                        ...
                        case 2: // components
                            ...
                            DialogMigrator.OpenUserMenu(iPlayer, Dialogs.menu_shop_ammunation_components);
                            break;
                        default:
                ...
                else if (menuid == Dialogs.menu_shop_ammunation_components)
                {
                    switch (index)
                    {
                        ...
                        case 2:
                            {
                                // pistol schall
                                if (!iPlayer.Container.CanInventoryItemAdded(1201, 1))
                                {
                                    iPlayer.SendNewNotification("Du hast keinen Platz fuer dieses Item!");
                                    return;
                                }
                                ...
                                iPlayer.Container.AddItem(1201, 1);
                                break;
                            }
                        case 3:
                            {
                                // pistol light
                                if (!iPlayer.Container.CanInventoryItemAdded(1221, 1))
                                {
                                    iPlayer.SendNewNotification("Du hast keinen Platz fuer dieses Item!");
                                    return;
                                }

                                if (!iPlayer.TakeMoney(3000))
                                {
                                    iPlayer.SendNewNotification("Du hast nicht genug Geld dabei!");
                                    return;
                                }

                                iPlayer.Container.AddItem(1221, 1);
                                break;
                            }
                        case 4:
                            {
                                // swpistol schall
                                if (!iPlayer.Container.CanInventoryItemAdded(1204, 1))
                                {
                                    iPlayer.SendNewNotification("Du hast keinen Platz fuer dieses Item!");
                                    return;
                                }
                                ...
                                iPlayer.Container.AddItem(1204, 1);
                                break;
                            }
                        case 5:
                            {
                                // swpistol light
                                if (!iPlayer.Container.CanInventoryItemAdded(1222, 1))
                                {
                                    iPlayer.SendNewNotification("Du hast keinen Platz fuer dieses Item!");
                                    return;
                                }
                                ...
                                iPlayer.Container.AddItem(1222, 1);
                                break;
                            }
                        default:
                            DialogMigrator.CloseUserMenu(player, Dialogs.menu_shop_ammunation);
                ...
                else if (menuid == Dialogs.menu_givelicenses)
                {
                    ...
                    Player xplayer = iPlayer.GetData("giveLic");
                            ...
                            if (CrimeModule.Instance.CalcJailTime(xPlayer.Crimes) > 0)
                            {
                                if (index == idx)
                                {
                                    if (xPlayer.IsOrtable(iPlayer))
                                    {
                                        ...
                                        if (iPlayer.IsNSADuty || iPlayer.FindFlags.HasFlag(FindFlags.Continuous))
                                        {
                    ...
                    switch (index)
                    {
                        ...
                        case 1: // Ticket
                            ...
                            if (iPlayer.Crimes.Count > 0)
                            {
                                ...
                                {
                                    l_Price = CrimeModule.Instance.CalcJailCosts(iPlayer.Crimes, iPlayer.EconomyIndex);
        ...
        private static void CreateUserMenuFahrzeugGarage(Player player, DbPlayer iPlayer, Garage garage)
        {
            DialogMigrator.CreateMenu(player, Dialogs.menu_garage_overlay, "Fahrzeug-Garage", "");
            DialogMigrator.AddMenuItem(player, Dialogs.menu_garage_overlay, MSG.General.Close(), "");
            DialogMigrator.AddMenuItem(player, Dialogs.menu_garage_overlay, "Fahrzeug entnehmen", "");
