...
using VMP_CNR.Module.Heist.Prequest.Keycard;
...
namespace VMP_CNR.Module.Doors
{
    ...
    public sealed class DoorModule : SqlModule<DoorModule, Door, uint>
    {
        ...
        public bool CanInteract(DbPlayer dbPlayer)
        {
            ...
            // Keycard
            if (door.Teams.Where(t => t.IsCops()).ToList().Count > 0 && KeycardModule.Instance.HasPlayerActiveKeycard(dbPlayer)) return true;

            if (Modules.Instance.HasDoorAccess(dbPlayer, door)) return true;

            if (door.Einreiseamt && dbPlayer.IsEinreiseAmt()) return true;
