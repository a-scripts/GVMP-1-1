...
namespace VMP_CNR.Module.Robbery
{
    ...
    public sealed class StaatsbankRobberyModule : Module<StaatsbankRobberyModule>
    {
        ...
        public string robname = "Staatsbank";
        ...
        public bool CanStaatsbankRobbed()
        {
            ...
            // Check other Robs
            if(RobberyModule.Instance.Robberies.Where(r => r.Value.Type == RobType.Juwelier && RobberyModule.Instance.IsActive(r.Value.Id)).Count() > 0 || VespucciBankRobberyModule.Instance.IsActive || LifeInvaderRobberyModule.Instance.IsActive)
            {
                return false;
            }
        ...
        public void StartRob(DbPlayer dbPlayer)
        {
            Main.m_AsyncThread.AddToAsyncThread(new Task(() =>
            {
                ...
                if (Instance.IsActive || RobberyModule.Instance.LastScenario.AddHours(2) > DateTime.Now || (LastStaatsbank.AddHours(2) > DateTime.Now && !Configurations.Configuration.Instance.DevMode))
                {
