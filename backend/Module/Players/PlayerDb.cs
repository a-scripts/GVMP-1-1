...
namespace VMP_CNR.Module.Players
{
    public static class PlayerDb
    {
        public static DbPlayer GetPlayer(this Player Player)
        {
            ...
            if (!Player.HasData("player")) return null;
            var dbPlayer = Player.GetData<DbPlayer>("player");
        ...
        public static void SaveWeapons(this DbPlayer iPlayer)
        {
            if (iPlayer.AccountStatus != AccountStatus.LoggedIn) return;
            ...
            update += " WHERE `id` = '" + iPlayer.Id + "';";

            if (update == "") return;
            if (!update.Contains("UPDATE")) return;
            MySQLHandler.ExecuteAsync(update);

            iPlayer.SaveWeaponComponents();
        }


        public static void SaveWeaponComponents(this DbPlayer iPlayer)
        {
            if (iPlayer.AccountStatus != AccountStatus.LoggedIn) return;
            ...
            string xstr;
            if ((xstr = Helper.Helper.GetWeaponComponents(iPlayer)) != "")
            {
                update += xstr;
            }
            else return;
        ...
        public static string GetUpdateQuery(this DbPlayer iPlayer, bool disconnect = false)
        {
            ...
            if (Helper.Helper.CheckPlayerData(iPlayer, iPlayer.Paintball, DbPlayer.Value.Paintball, out query))
            {
                ups.Add(query);
            }
            ...
            if ((xstr = Helper.Helper.ComplainPlayerDataInt(iPlayer.jailtimeReducing, "jailtime_reduce")) != "")
            {
                iPlayer.jailtimeReducing[1] = iPlayer.jailtimeReducing[0];
                ups.Add(xstr);
            }
            ...
            {
                if (!iPlayer.HasData("nsaChangedNumber")) 
                {
            ...
            if ((xstr = Helper.Helper.ComplainPlayerDataInt(iPlayer.zwd, "zwd")) != "")
            {
                iPlayer.zwd[1] = iPlayer.zwd[0];
                ups.Add(xstr);
            }
            ...
            if (iPlayer.DimensionType[0] != DimensionType.Paintball && iPlayer.DimensionType[0] != DimensionType.Gangwar && iPlayer.Paintball == 0)
            {
