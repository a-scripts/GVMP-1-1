...
namespace VMP_CNR.Module.Players
{
    public sealed class Players
    {
        ...
        public void SendNotificationToAllUsers(string command, int duration = 5000)
        {
            ...
            {
                foreach (var dbPlayer in players.Values)
                {
        ...
        public void SendMessageToHighTeam(string feature, string command, int time = 18000)
        {
            Main.m_AsyncThread.AddToAsyncThread(new Task(() =>
            {
                foreach (var dbPlayer in GetValidPlayers().Where(p => p.Rank.Id == 4 || p.Rank.Id == 5 || p.Rank.Id == 6 || p.Rank.Id == 8 ))
                {
                    if (dbPlayer == null || !dbPlayer.IsValid()) continue;
                    if (dbPlayer.Player.HasFeatureIgnored(feature)) continue;
        ...
        public DbPlayer Load(MySqlDataReader reader, Player player)
        {
            ...
            iPlayer.jailtimeReducing = new int[2];
            iPlayer.jailtimeReducing[1] = reader.GetInt32("jailtime_reduce");
            iPlayer.jailtimeReducing[0] = reader.GetInt32("jailtime_reduce");
            ...
            if (handy <= 0)
            {
                handy = 1275 + iPlayer.Id;
            }
            Console.WriteLine("FUNF");
            ...
            iPlayer.zwd = new int[2];
            iPlayer.zwd[1] = reader.GetInt32("zwd");
            iPlayer.zwd[0] = reader.GetInt32("zwd");
            ...
            Console.WriteLine("ACHT");

            // Add to Global Playerlist
            players.TryAdd(iPlayer.Id, iPlayer);
