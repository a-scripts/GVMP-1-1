...
namespace VMP_CNR.Module.Players
{
    public static class PlayerMoney
    {
        ...
        public static async Task<bool> TakeMoneyMenu(this DbPlayer player, int money)
        {
            player.Player.TriggerEvent("openMoneywindow", money);
            ...
            if (player.HasData("selected"))
            {
                if (player.GetData("selected") == "bar")
                {
                    if (money < 0) ja = false;
                    if (player.money[0] < money) ja = false;
                    if (player.money[0] - money > player.money[0]) ja = false;
                    player.money[0] = player.money[0] - money;
                    player.Player.TriggerEvent("updateMoney", player.money[0]);
                    player.Save();
                    ja = true;
                ...
                {
                    if (money < 0) ja = false;
                    if (player.bank_money[0] < money) ja = false;
                    if (player.bank_money[0] - money > player.bank_money[0]) ja = false;

                    player.bank_money[0] = player.bank_money[0] - money;
                    player.Save();
                    ja = true;
                }
                player.ResetData("selected");
            }
            else
            {
                player.SendNewNotification("Zeit ist abgelaufen!");
                player.ResetData("selected");
