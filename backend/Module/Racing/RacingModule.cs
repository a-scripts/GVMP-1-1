...
using VMP_CNR.Module.Racing.Menu;
using VMP_CNR.Module.Vehicles;

namespace VMP_CNR.Module.Racing
{
    public class RacingLobby
    {
        ...
        public RacingLobby(uint lobbyid, uint racingdimension, string desc = "")
        {
            LobbyId = lobbyid;
    ...
    {
        public static bool RacingDeactivated = true;
        ...
        public static Vector3 RacingMenuPosition = new Vector3(126.514, 6353.68, 31.3759);
        ...
        public override bool OnKeyPressed(DbPlayer dbPlayer, Key key)
        {
            ...
            if (dbPlayer.Dimension[0] == 0 && key == Key.E)
            {
                if(dbPlayer.Player.Position.DistanceTo(RacingMenuPosition) < 2.0f)
                {
                    ...
                    Module.Menu.MenuManager.Instance.Build(VMP_CNR.Module.Menu.PlayerMenu.RacingEnterMenu, dbPlayer).Show(dbPlayer);
                    return true;
                }
            }
                ...
                if(colShape.GetData<int>("racingColshape") == 1) // Start - End Colshape
                {
                    if(dbPlayer.HasData("racingState") && dbPlayer.GetData("racingState") == 10 && dbPlayer.HasData("racingRoundStartTime")) // has state 4 and is at start shape
                    {
                        ...
                        if(dbPlayer.RacingBestTimeSeconds == 0 || dbPlayer.RacingBestTimeSeconds > totalmil)
                        {
                            ...
                            dbPlayer.SendNewNotification($"1337Allahuakbar$racinbbesttime", duration: 16000);
                        }
                    }
