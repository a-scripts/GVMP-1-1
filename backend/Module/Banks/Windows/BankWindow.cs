...
namespace VMP_CNR.Module.Banks.Windows
{
    public class BankWindow : Window<Func<DbPlayer, string, string, int, int, int, List<BankHistory.BankHistory>, bool>>
    {
        ...
        [RemoteEvent]
        public void bankPayout(Player player, int balance)
        {
            BankTransaction(player, 0, balance);
        }

        [RemoteEvent]
        public void bankDeposit(Player player, int balance)
        {
            BankTransaction(player, balance, 0);
        }

        [RemoteEvent]
        public void bankTransfer(Player player, int amount, string target)
        {
        ...
        public void BankTransaction(Player player, int einzahlen, int auszahlen)
        {
            ...
            switch (iPlayer.DimensionType[0])
            {
                ...
                default:
                        ...
                        if (teamShelter != null)
                        {
                            ...
                            if(iPlayer.HasData("swbank"))
                            {
                                ...
                                if (einzahlen > 0 && einzahlen <= iPlayer.blackmoney[0])
                                {
                                    ...
                                    if (iPlayer.HasData("packBlackMoney"))
                                    {
                                        Logging.Logger.LogToAcDetections(iPlayer.Id, Logging.ACTypes.SWBankAbuse, $"BlackMoney {einzahlen} transfered while BM {iPlayer.GetData("packBlackMoney")} packed");
                                    }
                                }

                                iPlayer.ResetData("swbank");
                                return;
                            }
                            else
                            {
                    ...
                    if (iPlayer.TryData("bankId", out uint bankId))
                    {
                        ...
                        if (auszahlen > 0 && auszahlen <= iPlayer.bank_money[0])
                        {
                            if (iPlayer.TakeBankMoney(auszahlen))
                            {
                                ...
                                iPlayer.SendNewNotification(
                                    "Sie haben " + MSG.Money.fnumber(auszahlen) + "$ von Ihrem Konto abgehoben.", title: "Konto", notificationType: PlayerNotification.NotificationType.SUCCESS);
                        ...
                        if (einzahlen > 0 && einzahlen <= iPlayer.money[0])
                        {
                            if (iPlayer.GiveBankMoney(einzahlen))
                            {
                                ...
                                bank.SaveActMoneyToDb();
                                iPlayer.SendNewNotification(
                                    "Sie haben " + MSG.Money.fnumber(einzahlen) + "$ auf Ihr Konto eingezahlt.", title: "Konto", notificationType: PlayerNotification.NotificationType.SUCCESS);
                    ...
                    if (iPlayer.Player.Position.DistanceTo(PlayerDepotModule.DepotManagementPosition) < 1.5f)
                    {
                        if (!iPlayer.HasDepot()) return;
                        if (iPlayer.HasDepotLimitReached(PlayerDepotExtensions.DepotOperation.Both))
                        {
                            iPlayer.SendNewNotification($"Du hast bereits das tägliche Limit von {PlayerDepotModule.DepotDailyMaximum} $ für Ein- sowie Auszahlungen innerhalb deines Depots erreicht!");
                            return;
                        }

                        if (auszahlen > 0 && auszahlen <= iPlayer.Depot.Amount)
                        {
                            if (iPlayer.HasDepotLimitReached(PlayerDepotExtensions.DepotOperation.Subtract, (uint)auszahlen))
                            {
                                iPlayer.SendNewNotification($"Du hast bereits das tägliche Limit von {PlayerDepotModule.DepotDailyMaximum} $ für Auszahlungen innerhalb deines Depots erreicht!");
                                return;
                            }

                            uint tax = (uint)(auszahlen * 0.1);
                            iPlayer.Depot.Subtract((uint)auszahlen);
                            iPlayer.GiveMoney((int)(auszahlen - tax));
                            iPlayer.SendNewNotification($"Sie haben {auszahlen} $ aus Ihrem Depot ausgezahlt! Es wurden 10% Steuer ({tax} $) verrechnet!");
                            return;
                        }

                        if (einzahlen > 0 && einzahlen <= iPlayer.money[0])
                        {
                            if (iPlayer.HasDepotLimitReached(PlayerDepotExtensions.DepotOperation.Add, (uint)einzahlen))
                            {
                                iPlayer.SendNewNotification($"Du hast bereits das tägliche Limit von {PlayerDepotModule.DepotDailyMaximum} $ für Einzahlungen innerhalb deines Depots erreicht!");
                                return;
                            }

                            if (iPlayer.TakeMoney(einzahlen))
                            {
                                uint tax = (uint)(einzahlen * 0.1);
                                iPlayer.Depot.Add((uint)einzahlen - tax); // 10% weniger einzahlen zwecks Steuern
                                iPlayer.SendNewNotification($"Sie haben {einzahlen} $ in Ihr Depot eingezahlt! Es wurden 10% Steuer ({tax} $) verrechnet!");
                            }
                            return;
                        }
