...
namespace VMP_CNR.Module.Teamfight.Windows
{
    public class TeamfightWindow : Window<Func<DbPlayer, uint, string, bool, bool>>
    {
        ...
        [RemoteEvent]
        public void requestTeamfight(Player player, uint id, int money)
        {
            Main.m_AsyncThread.AddToAsyncThread(new System.Threading.Tasks.Task(() =>
            {
                ...
                if (!enemy.IsInTeamfight())
                {
                    ...
                    if (enemy.GetTeamMembers().Where(c => c.TeamRankPermission.Manage > 1).Count() > 0)
                    {
                        foreach (var findplayer in enemy.GetTeamMembers().Where(c => c.TeamRankPermission.Manage > 1))
                        {
                            findplayer.SendNewNotification("Es ist eine Anfrage zu einem Fraktionskampf eingegangen, bitte beantworte diese in deinem Fraktionslager!");
                        }
                    }
                    else
                    {
