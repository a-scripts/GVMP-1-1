...
namespace VMP_CNR.Module.NSA.Observation
{
    public class NSAObservationModule : SqlModule<NSAObservationModule, NSADBObservation, uint>
    {
        ...
        public void AgreeObservation(DbPlayer iPlayer, NSAObservation nSAObservation)
        {
            ...
            iPlayer.SendNewNotification($"Sie haben die Observation von {PlayerNameModule.Instance.Get((uint)nSAObservation.PlayerId).Name} genehmigt!");
        }
        ...
        {
            foreach(DbPlayer xPlayer in NSAModule.Instance.NSAMember.ToList().Where(p => p.HasData("nsa_activePhone") && p.GetData("nsa_activePhone") == number).ToList()) 
            {
