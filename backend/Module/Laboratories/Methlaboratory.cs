...
namespace VMP_CNR.Module.Laboratories
{
    public class Methlaboratory : Loadable<uint>
    {
        ...
        {
            MySQLHandler.ExecuteAsync($"UPDATE team_methlaboratories SET last_attacked = '{LastAttacked.Date.ToString("yyyy-MM-dd HH:mm:ss")}' WHERE id = '{Id}'");
        }
        ...
        public async Task<bool> ImpoundMethlaboratory(DbPlayer dbPlayer)
        {
            ...
            if (!(LaboratoryModule.Instance.IsImpoundVehicle(closestVeh))
                    ...
                    (!closestVeh.HasData("Door_KRaum") || 
                    closestVeh.GetData("Door_KRaum") != 1) && closestVeh.Container != null)
            {
            ...
            if (!LaboratoryModule.Instance.IsImpoundVehicle(closestVeh) || closestVeh.teamid != dbPlayer.TeamId ||
                    !closestVeh.HasData("Door_KRaum") || closestVeh.GetData("Door_KRaum") != 1 || closestVeh.Container == null)
            {
                dbPlayer.SendNewNotification("Du benötigst ein Lagerfahrzeug (Burrito, Brickade, ...) mit offenem Kofferraum.");
                ImpoundInProgress = false;
                return false;
