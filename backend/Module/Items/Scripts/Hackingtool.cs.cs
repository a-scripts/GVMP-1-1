...
namespace VMP_CNR.Module.Items.Scripts
{
    public static partial class ItemScript
    {
        public static async Task<bool> Hackingtool(DbPlayer iPlayer, ItemModel ItemData)
        {
            ...
            {
                Module.Menu.MenuManager.Instance.Build(VMP_CNR.Module.Menu.PlayerMenu.HackingVoltageMenu, iPlayer).Show(iPlayer);
                return false;
            }

            // Vespucci Bank
            if (iPlayer.Player.Position.DistanceTo(LifeInvaderRobberyModule.Instance.RobPosition) < 2.0f)
            {
                await LifeInvaderRobberyModule.Instance.StartRob(iPlayer);
                return true;
            }
