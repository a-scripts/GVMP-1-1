...
namespace VMP_CNR.Module.Items
{
    public static class ContainerManager
    {
        ...
        public static Container CreateContainer(uint id, ContainerTypes type, int tempDefaultSlots = 0, int tempDefaultWeight = 0)
        {
            ...
            MySQLHandler.ExecuteAsync(query, Sync.MySqlSyncThread.MysqlQueueTypes.Inventory);
        ...
        public static bool IsItemRestrictedForContainer(this Container container, ItemModel item, DbPlayer dbPlayer = null)
        {
            ...
            if (item.Id == 737) return true; // kleidertasche nirgens hin...
            ...
            if (container.Type == ContainerTypes.VEHICLE2 || container.Type == ContainerTypes.FVEHICLE2)
            {
                // 640 - Schwarzgeld
                // 881 - Markierte Scheine
                if (item.Id == 640 || item.Id == 881) 
                    return true;
            }
        ...
        public static void ShowFriskInventory(this Container container, DbPlayer dbPlayer, DbPlayer target, string optionalname = " ", int money = 0)
        {
            ...
            containerList.Add(container.ConvertForPlayer(1, "Inventar von " + optionalname?.Replace('_', ' '), money));
        ...
        public static void ShowVehFriskInventory(this Container container, DbPlayer dbPlayer, string optionalname = " ")
        {
            ...
            List<PlayerContainerObject> containerList = new List<PlayerContainerObject>();
            containerList.Add(container.ConvertForPlayer(1, "Inventar von " + optionalname?.Replace('_', ' ')));
            ComponentManager.Get<InventoryWindow>().Show()(dbPlayer, containerList);
        }
        ...
        public static void ShowHouseFriskInventory(this Container container, DbPlayer dbPlayer, uint dimension)
        {
            ...
            List<PlayerContainerObject> containerList = new List<PlayerContainerObject>();
            containerList.Add(container.ConvertForPlayer(1, "Haus Inventar"));
            ComponentManager.Get<InventoryWindow>().Show()(dbPlayer, containerList);
        }

        public static PlayerContainerObject ConvertForPlayer(this Container container, int sendId, string optionalname = "", int money = 0, int blackmoney = 0, bool playerActions = false)
        {
            if (container == null) return null;
            PlayerContainerObject PlayerContainerObject = new PlayerContainerObject(container.MaxWeight, container.MaxSlots);

            PlayerContainerObject.Slots = new List<PlayerContainerSlotObject>();
            ...
            try
            {
                foreach (KeyValuePair<int, Item> kvp in container.Slots.ToList())
                {
                    if (kvp.Value != null && kvp.Value.Amount > 0)
                    {
                        ...
                        PlayerContainerObject.Slots.Add(new PlayerContainerSlotObject((int)firstItem.Id, kvp.Key, firstItem.Model, firstItem.Data, kvp.Value.Amount));
                    }
                }
            }
            catch (Exception e)
            ...
            PlayerContainerObject.Id = sendId;
            PlayerContainerObject.Name = optionalname != "" ? optionalname : ContainerMetaData.metaDataDic[container.Type].InventoryName;
            PlayerContainerObject.Money = money;
            PlayerContainerObject.Blackmoney = blackmoney;
            PlayerContainerObject.PlayerInventory = playerActions;

            return PlayerContainerObject;
        }
        ...
        public static int GetInventoryUsedSpace(this Container container)
        {
            ...
            {
                if (items.Value == null || items.Value.Id == 0 || items.Value.Amount == 0 || items.Value.Model == null) continue;
        ...
        public static bool RemoveIllegalItems(this Container container)
        {
            ...
            if (l_ContainerSaveQuery != "")
                MySQLHandler.ExecuteAsync(l_ContainerSaveQuery, Sync.MySqlSyncThread.MysqlQueueTypes.Inventory);

            return true;
        }

        public static bool ClearInventory(this Container container, bool disablesave = false)
        {
            List<int> touchedSlots = new List<int>();
        ...
        public static int GetMaxItemAddedAmount(this Container container, ItemModel itemModel)
        {
            ...
            if (itemModel.Weight == 0 || itemModel.Weight == 1)
                weightAmount = container.GetInventoryFreeSpace();
            else
        ...
        public static bool AddItem(this Container container, uint itemId, int amount = 1, Dictionary<string, dynamic> data = null, int slot = -1, bool disablesave = false)
        {
            ...
            if (container.Type == ContainerTypes.PLAYER)
            {
                DbPlayer dbPlayer = Players.Players.Instance.FindPlayer(container.Id);
        ...
        public static void MoveItemInContainer(this Container container, ItemModel ItemModel, int amount = 1, Dictionary<string, dynamic> data = null, int sourceSlot = -1, int destinationSlot = -1, bool disablesave = false)
        {
            if (ItemModel == null) return;

            //Remove Old
            RemoveItemSlotFirst(container, ItemModel, sourceSlot, amount, true);
            AddContainerItem(container, ItemModel, amount, data, destinationSlot, true);

            string l_ContainerSaveQuery = "";
            l_ContainerSaveQuery += container.GetSlotSaveQuery(sourceSlot);
            l_ContainerSaveQuery += container.GetSlotSaveQuery(destinationSlot);
        ...
        public static void RemoveItemSensetive(this Container container, Item item, bool disablesave = false)
        {
            ...
            foreach (KeyValuePair<int, Item> kvp in container.Slots.ToList())
            {
                if (kvp.Value != null && kvp.Value == item)
                {
                    container.SetSlotClear(kvp.Key);
        ...
        public static void RemoveItemSlotFirst(this Container container, ItemModel model, int slot, int amount = 1, bool disableSave = false)
        {
        ...
        public static void RemoveAllFromSlot(this Container container, int slot, bool disablesaving = false)
        {
        ...
        public static bool ClearOfflineInvDBOnly(ContainerTypes type, uint containerId)
        {

            string saveQuery = "";

            for (int i = 0; i < ContainerMetaData.metaDataDic[type].DefaultSlots; i++)
            {
                saveQuery += $"UPDATE `{GetTableName(type)}` SET `slot_{i}` = ''";
                saveQuery += $" WHERE `id` = '{containerId}';";
            }
        ...
        public static void SaveAll(this Container container)
        {
            ...
            {
                if (!container.Slots.ContainsKey(i)) continue;
        ...
        public static Item GetAttachmentOnlyItem(this Container container)
        {
            foreach (KeyValuePair<int, Item> kvp in container.Slots.ToList())
            {
                if (kvp.Value.Model != null && kvp.Value.Model.AttachmentOnlyId > 0)
                {
                    return kvp.Value;
                }
            }
        ...
        public static Item GetItemByIdAndData(this Container container, int id, string datakey)
        {
            foreach (KeyValuePair<int, Item> kvp in container.Slots.ToList())
            {
                if (kvp.Value.Id == id && kvp.Value.Data != null && kvp.Value.Data.ContainsKey(datakey))
                {
                    return kvp.Value;
                }
            }
