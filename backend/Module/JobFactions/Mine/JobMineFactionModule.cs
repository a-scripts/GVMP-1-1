            ...
            if(Mine1 != null)
            {
                if (Mine1.MineContainerSchmelze != null && Mine1.MineContainerSchmelzCoal != null)
                {
                    if (Mine1.MineContainerSchmelzCoal.GetItemAmount(Coal) >= 15)
                    {
                        ...
                        // Eisen
                        if (Mine1.MineContainerSchmelze.CanInventoryItemAdded(IronBarren, amountToConvert/5))
                        {
                            Mine1.MineContainerSchmelze.RemoveItem(IronErz, amountToConvert, true);
                            Mine1.MineContainerSchmelze.AddItem(IronBarren, amountToConvert/5, new Dictionary<string, dynamic>(), -1, true);
                        }

                        amountToConvert = Mine1.MineContainerSchmelze.GetItemAmount(Copper);
                        if (amountToConvert > (200 * playerCountOnline)) amountToConvert = (200 * playerCountOnline);
                        // Bronce
                        if (Mine1.MineContainerSchmelze.CanInventoryItemAdded(BronceBarren, amountToConvert/5))
                        {
                            Mine1.MineContainerSchmelze.RemoveItem(Copper, amountToConvert, true);
                            Mine1.MineContainerSchmelze.AddItem(BronceBarren, amountToConvert/5, new Dictionary<string, dynamic>(), -1, true);
                        }

                        amountToConvert = Mine1.MineContainerSchmelze.GetItemAmount(AluErz);
                        if (amountToConvert > (200 * playerCountOnline)) amountToConvert = (200 * playerCountOnline);
                        // Alu
                        if (Mine1.MineContainerSchmelze.CanInventoryItemAdded(AluBarren, amountToConvert/5))
                        {
                            Mine1.MineContainerSchmelze.RemoveItem(AluErz, amountToConvert, true);
                            Mine1.MineContainerSchmelze.AddItem(AluBarren, amountToConvert/5, new Dictionary<string, dynamic>(), -1, true);
                        }
