                ...
                using (var reader = cmd.ExecuteReader())
                {
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            dbPlayer.ActiveBusiness = business;

                            if (dbPlayer.ActiveBusiness.Members.ContainsKey(dbPlayer.Id))
                                dbPlayer.ActiveBusiness.Members.Remove(dbPlayer.Id);

                            dbPlayer.ActiveBusiness.Members.Add(dbPlayer.Id, new Business.Member(reader));
                            dbPlayer.BusinessMembership = dbPlayer.ActiveBusiness.Members[dbPlayer.Id];
                            break;
                        }
                    }
                }
