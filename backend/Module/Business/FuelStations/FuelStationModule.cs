...
namespace VMP_CNR.Module.Business.FuelStations
{
    ...
    {
        public static List<uint> AirportFuels = new List<uint>() { 33, 29, 27, 25 };
        public static List<uint> AirVehicleDataGroups = new List<uint>() { 8, 9 };
        ...
        protected override void OnItemLoaded(FuelStation fuelStation)
        {
            ...
            // Set Marker lul
            Main.ServerBlips.Add(Blips.Create(fuelStation.Position, "", 361, 1.0f, true, 64));
            return;
        }
        ...
        public override bool OnKeyPressed(DbPlayer dbPlayer, Key key)
        {
            ...
            else
            {
                ...
                if(fuel != null)
                {
                    ...
                    if (!AirportFuels.Contains(fuel.Id) && AirVehicleDataGroups.Contains(sxVehicle.Data.ClassificationId)) // flugzeuge an anderen tanken raus
                    {
                        return false;
                    }

                    if (AirportFuels.Contains(fuel.Id) && !AirVehicleDataGroups.Contains(sxVehicle.Data.ClassificationId)) // airport und kein heli/flugzeug
                    {
                        return false;
                    }
