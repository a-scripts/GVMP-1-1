...
using VMP_CNR.Module.Heist.Planning.Menu;
using VMP_CNR.Module.Items;
using VMP_CNR.Module.Laboratories.Menu;
using VMP_CNR.Module.Menu;
using VMP_CNR.Module.Menu.Menus.Heists.Planningroom;
...
namespace VMP_CNR.Module.Heist.Planning
{
    public class PlanningModule : SqlModule<PlanningModule, PlanningRoom, uint>
    {
        ...
        public Vector3 PlanningroomHeistInfoLocation = new Vector3(2712.92, -366.776, -54.7809);
        ...
        protected override void OnLoaded()
        {
            ...
            MenuManager.Instance.AddBuilder(new PlanningroomHeistInfoMenu());


            MenuManager.Instance.AddBuilder(new PlanningroomWeaponCreateMenuBuilder());
        ...
        public override bool OnKeyPressed(DbPlayer dbPlayer, Key key)
        {
            ...
            if (dbPlayer.Player.Position.DistanceTo(PlanningroomHeistInfoLocation) < 2.0f && room.BasementMechanicLevel == 1 && room.BasementWardrobeLevel == 1 && room.BasementWeaponsLevel == 1 && room.BasementWeaponsLevel == 1)
            {
                return InfotableMenu(dbPlayer);
            }
            ...
            else if (dbPlayer.Player.Position.DistanceTo(PlanningRoomWeaponLocation) < 2.0f && room.BasementWeaponsLevel == 1)
            {
                ...
                // Open Menu
                MenuManager.Instance.Build(PlayerMenu.PlanningroomWeaponCreateMenu, dbPlayer).Show(dbPlayer);
                return false;
            }
        ...
        private bool InfotableMenu(DbPlayer dbPlayer)
        {
            MenuManager.Instance.Build(PlayerMenu.PlanningroomInfoTableMenu, dbPlayer).Show(dbPlayer);
            return false;
        }
