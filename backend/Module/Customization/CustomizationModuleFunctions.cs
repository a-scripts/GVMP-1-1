...
namespace VMP_CNR.Module.Customization
{
    public static class CustomizationModuleFunctions
    {
        ...
        public static void ApplyCharacter(this DbPlayer iPlayer, bool loadArmorHealthFromDBValue = false, bool ignoreOutfit = false)
        {
            NAPI.Task.Run(() =>
            {
                if (!iPlayer.Firstspawn && !loadArmorHealthFromDBValue) iPlayer.UpdatePlayerHealthAndArmor();
                ...
                {
                    iPlayer.SetSkin((PedHash)HalloweenModule.Instance.GetZombieSkinRandom());
                    return;
                }

                if (iPlayer.Customization == null) iPlayer.Customization = new CharacterCustomization();

                if ((PedHash)iPlayer.Player.Model != PedHash.FreemodeFemale01 && (PedHash)iPlayer.Player.Model != PedHash.FreemodeMale01) iPlayer.SetSkin(iPlayer.Character.Skin);
                ...
                ClothModule.Instance.RefreshPlayerClothes(iPlayer, ignoreOutfit);
                ...
                // Set to fist
                iPlayer.GiveServerWeapon(WeaponHash.Unarmed, 1);

                iPlayer.ApplyPlayerHealth();

                //Fix not visible Bug?
                iPlayer.Player.Transparency = 255;

                // Warum doppelt aufrufen? @Jeff
                //AttachmentModule.Instance.RemoveAllAttachments(iPlayer);
        ...
        public static void StartCustomization(this DbPlayer iPlayer)
        {
            ...
            iPlayer.SetData("lastArmor", player.Armor);

            iPlayer.SetDimension(iPlayer.Id);
        ...
        public static async Task StopCustomization(this DbPlayer iPlayer)
        {
            var player = iPlayer.Player;
            ...
            if (iPlayer.NeuEingereist())
            {
                ...
                player.TriggerEvent("startWelcomeCutscene", iPlayer.Customization.Gender, iPlayer.Player.Name);
            ...
            if (iPlayer.HasData("lastPosition"))
            {
                ...
                {
                    iPlayer.SetDimension(iPlayer.GetData("lastDimension"));
                }
            ...
            if (iPlayer.HasData("lastArmor")) 
            {
                iPlayer.SetArmorPlayer(iPlayer.GetData("lastArmor"));
            }
        }
    }
}
