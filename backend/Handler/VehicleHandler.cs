...
namespace VMP_CNR.Handler
{
    public sealed class VehicleHandler : Module<VehicleHandler>
    {
        ...
        {
            List<SxVehicle> sxVehicles = new List<SxVehicle>();
            sxVehicles = GetTeamVehicles((uint)teamid).Where(sxVeh => sxVeh.entity.Position.DistanceTo(position) <= range && !sxVeh.SyncExtension.Locked && sxVeh.HasData("Door_KRaum") && sxVeh.GetData("Door_KRaum") == 1).ToList();
            return sxVehicles;
        }
        ...
        public void AddPlayerToVehicleOccupants(SxVehicle sxVehicle, DbPlayer dbPlayer, int seat)
        {
            try
            {
                ...
                if (occupants.ContainsValue(dbPlayer))
                {
                    ...
                    foreach(KeyValuePair<int, DbPlayer> kvp in occupants.ToList())
                    {
                        if(kvp.Value != null && kvp.Value.IsValid() && kvp.Value == dbPlayer)
                        {
                            playerseat = kvp.Key;
                        }
                    }
            ...
            int WheelClamp = 0, bool AlarmSystem = false, uint lastgarageId = 0, bool planningvehicle = false, int carSellPrice = 0, Container container2 = null)
        {
            try
            {
                ...
                xVeh.SetData("Door_KRaum", 0);
                // Set Anticheat Data
                xVeh.entity.SetData("serverhash", "1312asdbncawssd1ccbSh1");
                ...
                return xVeh;
            } catch (Exception ex) { Logger.Print("createservervehiuckele: " + ex.ToString()); return new SxVehicle(); }
        }
    ...
    public class SxVehicle
    {
        ...
        public Dictionary<int, DbPlayer> GetOccupants()
        {
            try
            {
                Dictionary<int, DbPlayer> returnOccuPants = new Dictionary<int, DbPlayer>();

                foreach (DbPlayer kvp in Occupants.Values.ToList())
                {
                    if (kvp != null && kvp.IsValid() && kvp.Player.IsInVehicle)
                    {
                        // recheck Vehicle
                        SxVehicle checkVeh = kvp.Player.Vehicle.GetVehicle();
                        if (checkVeh != null && checkVeh.IsValid() && checkVeh.databaseId == databaseId)
                        {
                            returnOccuPants.TryAdd(kvp.Player.VehicleSeat, kvp);
                        }
                    }
                }

                // overwrite
                Occupants = returnOccuPants;
            ...
            {
                Logger.Crash(e);
                return new Dictionary<int, DbPlayer>();
            }
        }
